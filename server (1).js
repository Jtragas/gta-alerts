...
const ALERT_EXPIRY_TIME = 6 * 60 * 60 * 1000; // 6 hours - alerts expire after this
...
async function fetchBreakingAlerts() {
    ...
    // Remove alerts older than 6 hours
    const now = Date.now();
    const freshAlerts = flatAlerts.filter(alert => {
        const alertTime = new Date(alert.publishedAt).getTime();
        const age = now - alertTime;
        return age < ALERT_EXPIRY_TIME;
    });
...
async function start() {
    ...
    app.listen(PORT, () => {
        ...
        console.log(`🚨 Breaking GTA Alerts: ACTIVE (6hr expiry)`);
    });
}
