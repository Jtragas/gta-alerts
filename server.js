    ...
    try {
        ...
        const incidents = events.map((event, i) => {
            ...
            if (event.dispatch_time) {
                try {
                    timestamp = new Date(event.dispatch_time).toISOString();
                } catch (e) {
        ...
        const incidents = response.data.features
            ...
            .map((feature, i) => {
                ...
                if (attrs.OCCURRENCE_TIME_AGOL) {
                    timestamp = new Date(attrs.OCCURRENCE_TIME_AGOL).toISOString();
                }
                ...
                const description = division ? `Division ${division} - ${callType}` : callType;
